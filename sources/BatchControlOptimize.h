 ...
 * Case<n>.m, Case<n>.txt, Case<n>.txt_RM_BFSResult.txt,
 * Case<n>.txt_RM_GPResult.txt
 *
 * @param file
 * @return int
...
/**
 * @brief read baseline result file, e.g., Case0.txt_RM_GPResult.txt, or
 * Case0.txt_RM_BFSResult.txt
 *
 * @param directory absolute folder directory
 * @param path name of target file
 * @return pair<double, double> {runTime, obj}
 */
std::pair<double, double> ReadBaselineZhao20(std::string directory,
                                             std::string path) {
    std::fstream file;
    ...
    if (file.is_open()) {
        ...
        } else  // return initial estimate
        {
            if (TargetFileType(path) == 2)
                CoutWarning("Find one unschedulable baseline assignment: " +
                            path);
...
void BatchOptimize(int Nn = 5) {
    ...
    for (int i = 0; i < 3; i++) objVecAll.push_back(dummy);
        ...
        switch (type) {
            case 0:  // perform optimization
            {
                ...
                std::vector<bool> maskForElimination(
                    tasks.size(), false);  // TODO: try *2 to ?
                ResetCallingTimes();
            ...
            case 2:  // read MUA result
            {
                ...
                double relativeGapCurr =
                    (objVecAll[0].back() - objVecAll[2].back()) /
                    objVecAll[2].back();
    ...
    std::cout << "Average relative performance gap (NO: MUA) is "
              << RelativeGap(objVecAll[0], objVecAll[2]) << std::endl;

    std::cout << "Speed ratio (NO: MUA) is "
              << SpeedRatio(runTimeAll[0], runTimeAll[2]) << std::endl;
    std::cout << "Average time consumed is " << Average(runTimeAll[0])
              << std::endl;
    ...
    pathRes = "/home/zephyr/Programming/Energy_Opt_NLP/CompareWithBaseline/" +
              batchOptimizeFolder + "/RTACalling/N" + std::to_string(Nn) +
              ".txt";
    AddEntry(pathRes, aveRTA);
    std::cout << "Average rta calling times is " << aveRTA << std::endl;
    if (printFailureFile) {
        std::cout << std::endl;
        for (auto &file : failedFiles) std::cout << file << std::endl;
    }
}
}  // namespace rt_num_opt