...
std::vector<rt_num_opt::DAG_Model> ReadDAG_NasriFromYaml(std::string path) {
    ...
    for (size_t i = 0; i < tasksNode.size(); i++) {
        ...
        for (size_t j = 0; j < edgesNode.size(); j++) {
            // std::cout << "Add edge: " << edgesNode[j]["from"].as<int>() << ",
            // " << edgesNode[j]["to"].as<int>() << std::endl;
            dag.AddEdge(edgesNode[j]["from"].as<int>(),
                        edgesNode[j]["to"].as<int>());
        }

        dags.push_back(dag);
    }
    sort(dags.begin(), dags.end(),
         [](const rt_num_opt::DAG_Model &a, const rt_num_opt::DAG_Model &b)
             -> bool { return a.tasks_[0].deadline < b.tasks_[0].deadline; });
    return dags;
}

VectorDynamic ReadControlCoeff(std::string path) {
    YAML::Node config = YAML::LoadFile(path);
    YAML::Node coeff_node;
    if (config["coeff_control"]) {
        coeff_node = config["coeff_control"];
    } else {
        CoutError("Input file doesn't store coeff_control: " + path);
    }
    std::vector<double> coeff_vec;
    coeff_vec.reserve(coeff_node.size());
    for (size_t i = 0; i < coeff_node.size(); i++)
        coeff_vec.push_back(coeff_node[i].as<int>());
    return Eigen::Map<Eigen::VectorXd>(coeff_vec.data(), coeff_vec.size());
}

void WriteDAG_NasriToYaml(std::vector<rt_num_opt::DAG_Model> &dags,
                          std::string path, const VectorDynamic &coeff) {
    YAML::Node nodeTasks;
    for (size_t i = 0; i < dags.size(); i++) {
        ...
        for (size_t j = 0; j < dag.tasks_.size(); j++) {
            ...
            taskNode["executionTime"] =
                std::to_string(int(dag.tasks_[j].executionTime));
    ...
    nodeRoot["coeff_control"] =
        std::vector<double>(coeff.data(), coeff.data() + coeff.rows());
