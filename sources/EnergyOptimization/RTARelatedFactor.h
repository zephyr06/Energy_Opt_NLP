...
//     RTARelatedFactor(std::vector<gtsam::Symbol> &keyVec, TaskSetType &tasks,
//     int index, VectorDynamic rtaBase,
//                      gtsam::SharedNoiseModel model) :
//                      gtsam::NoiseModelFactor(model, keyVec), tasks(tasks),
//                      index(index), rtaBase(rtaBase),
//                      dimension(keyVec.size()), keyVec(keyVec)
//     {
...
//             TaskSetType tasksCurr = tasks;
//             UpdateTaskSetExecutionTime(tasksCurr,
//             EnergyOptUtils::ExtractResults(x, tasks)); Schedul_Analysis
//             r(tasksCurr); double rta = r.RTA_Common_Warm(rtaBase(index),
//             index); error(0) = Barrier(min(tasksCurr[index].period,
//             tasksCurr[index].deadline) - rta);

//             eliminationRecordGlobal.AdjustEliminationError(error(0), index,
//             EliminationType::RTA); EndTimer("f_with_RTA"); return error;
//         };
//     }
...
//                         return f_with_RTA(xx);
//                     };
//                     (*H)[i] = NumericalDerivativeDynamic(f,
//                     x.at<VectorDynamic>(keyVec[i]), deltaOptimizer);
//                 }
//                 else
//                     (*H)[i] = GenerateVectorDynamic(1);
//             }
...
// template <class TaskSetType, class Schedul_Analysis>
// RTARelatedFactor<TaskSetType, Schedul_Analysis>
// GenerateRTARelatedFactor(TaskSetType &tasks, int index)
// {
...
//     return RTARelatedFactor<TaskSetType, Schedul_Analysis>(keys, tasks,
//     index, model);
// }
