...
namespace dagSched
{
    ...
    enum GenerationType_t
    {
        VARYING_N,
        VARYING_U,
        VARYING_M
    };
    enum DeadlinesType_t
    {
        CONSTRAINED,
        IMPLICIT,
        ARBITRARY
    ...
    public:
        ...
        void configureParams(GenerationType_t gt)
        {
            ...
            if (DAGType != DAGType_t::TDAG)
            {
                ...
                for (size_t p = 0; p < typedProc.size(); ++p)
        ...
        void readFromYaml(const std::string &params_path)
        {
            ...
            if (config["maxCondBranches"])
                maxCondBranches = config["maxCondBranches"].as<int>();
            if (config["maxParBranches"])
                maxParBranches = config["maxParBranches"].as<int>();
            if (config["recDepth"])
                recDepth = config["recDepth"].as<int>();
            if (config["pCond"])
                pCond = config["pCond"].as<float>();
            if (config["pPar"])
                pPar = config["pPar"].as<float>();
            if (config["pTerm"])
                pTerm = config["pTerm"].as<float>();
            if (config["Cmin"])
                Cmin = config["Cmin"].as<float>();
            if (config["Cmax"])
                Cmax = config["Cmax"].as<float>();
            if (config["addProb"])
                addProb = config["addProb"].as<float>();
            if (config["probSCond"])
                probSCond = config["probSCond"].as<float>();
            if (config["Utot"])
                Utot = config["Utot"].as<float>();
            if (config["beta"])
                beta = config["beta"].as<float>();
            if (config["saveRate"])
                saveRate = config["saveRate"].as<int>();
            if (config["Umin"])
                Umin = config["Umin"].as<float>();
            if (config["Umax"])
                Umax = config["Umax"].as<float>();
            if (config["stepU"])
                stepU = config["stepU"].as<float>();
            if (config["m"])
                m = config["m"].as<int>();
            if (config["nMin"])
                nMin = config["nMin"].as<int>();
            if (config["nMax"])
                nMax = config["nMax"].as<int>();
            if (config["nTasks"])
                nTasks = config["nTasks"].as<int>();
            if (config["stepN"])
                stepN = config["stepN"].as<int>();
            if (config["mMin"])
                mMin = config["mMin"].as<int>();
            if (config["mMax"])
                mMax = config["mMax"].as<int>();
            if (config["stepM"])
                stepM = config["stepM"].as<int>();
            if (config["tasksetPerVarFactor"])
                tasksetPerVarFactor = config["tasksetPerVarFactor"].as<int>();
            if (config["nTasksets"])
                nTasksets = config["nTasksets"].as<int>();
            if (config["diffProcTypes"])
                diffProcTypes = config["diffProcTypes"].as<int>();
            if (config["minProcPerType"])
                minProcPerType = config["minProcPerType"].as<int>();
            if (config["maxProcPerType"])
                maxProcPerType = config["maxProcPerType"].as<int>();
            if (config["gType"])
                gType = (GenerationType_t)config["gType"].as<int>();
            if (config["dtype"])
                dtype = (DeadlinesType_t)config["dtype"].as<int>();
            if (config["sType"])
                sType = (SchedulingType_t)config["sType"].as<int>();
            if (config["aType"])
                aType = (AlgorithmType_t)config["aType"].as<int>();
            if (config["wType"])
                wType = (workloadType_t)config["wType"].as<int>();
            if (config["DAGType"])
                DAGType = (DAGType_t)config["DAGType"].as<int>();
        }
