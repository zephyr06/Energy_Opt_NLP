...
namespace dagSched
{
    ...
    std::string convertTasksetToCsv(Taskset &taskset)
    {
        ...
        std::ofstream Weather_test_output("path_goes_here_task.csv", std::ofstream::out | std::ofstream::trunc);
        // this does the open for you, appending data to an existing file
        Weather_test_output << task_set << std::endl;
    ...
    std::string convertDAGsToCsv(const Taskset &taskset)
    {
        ...
        std::ofstream Weather_test_output("path_goes_here_dag.csv", std::ofstream::out | std::ofstream::trunc);
        // this does the open for you, appending data to an existing file
        Weather_test_output << prec << std::endl;
        Weather_test_output.close();
        return prec;
    }
    ...
    std::vector<double> RTA_G_LP_FTP_Nasri2019_C(Taskset taskset, const int m)
    {
        ...
        if (space.is_schedulable())
        {
            ...
            for (const auto &j : problem.jobs)
            {
                ...
                // std::cout << "[" << j.get_task_id() << ", " << j.get_job_id() << "] ";
                // std::cout << std::max<long long>(0, (finish.from() - j.earliest_arrival())) << " ";
                res.push_back(std::max<long long>(0, (finish.from() - j.earliest_arrival())));
                // std::cout<<(finish.until() - j.earliest_arrival())<<" ";
            }
            // std::cout << std::endl;
        }
