...
namespace dagSched
{
    ...
    float computeSI(const std::vector<int> &path, const DAGTask &task)
    {
        ...
        if (METHOD_VERBOSE)
            printVector<int>(self, "self computeSI");
        ...
        for (int y = 0; y < task_idx; ++y)
        {
            ...
            for (int j = 0; j < V.size(); ++j)
            {
                if (V[j]->core == core_id)
                {
                    ...
                    if (METHOD_VERBOSE)
                        std::cout << "maxinterf --> y: " << y << ", " << j << " " << V[j]->r << " " << R_bar << std::endl;
                }
            }
    ...
    float Theorem1Casini2018(const SSTask &tau_ss, const Taskset &taskset, const int task_idx, const float SI)
    {
        ...
        if (METHOD_VERBOSE)
            printVector<float>(multiset_B, "multiset_B");
        b = 0;
    ...
    float Theorem2Casini2018(const int k, const SSTask &tau_ss, const std::vector<float> &R_ss, const Taskset &taskset, const int task_idx, const float SI)
    {
        ...
        if (METHOD_VERBOSE)
            std::cout << "R: " << R << " bI: " << bI << " SI: " << SI << std::endl;
        //equation 7
        return R + bI;
    }
    ...
    float pathAnalysisImproved(const std::vector<int> &path_ss, const Taskset &taskset, const int task_idx, std::vector<std::vector<float>> &RTs, const bool is_root)
    {
        //algorithm 5
        if (METHOD_VERBOSE)
            std::cout << "starting path analysis improved" << std::endl;
        if (METHOD_VERBOSE)
            printVector<int>(path_ss, "path ss");
        ...
        if (path_ss.size() == 1)
        {
            ...
            if (METHOD_VERBOSE)
                std::cout << "RT " << first->id << ", " << last->id << ": " << R_task_ss << std::endl;
        }
        else
        {
            if (first->core == last->core)
            {
                ...
                if (METHOD_VERBOSE)
                    std::cout << "RT " << first->id << ", " << last->id << ": " << RTs[first->id][last->id] << std::endl;
            }
            else
            {
        ...
        if (is_root)
        {
            ...
            if (METHOD_VERBOSE)
                printSet<int>(path_cores, "path_cores");
        ...
        while (at_least_one_update)
        {
            ...
            for (int x = 0; x < taskset.tasks.size(); ++x)
            {
                ...
                for (const auto &p : all_paths)
                {
                    ...
                    if (METHOD_VERBOSE)
                        std::cout << "taskset.tasks[" << x << "].R " << taskset.tasks[x].R << std::endl;

                    for (const auto &v : p)
                    {
                        if (METHOD_VERBOSE)
                            std::cout << "RT vv: " << RTs[v][v] << std::endl;
            ...
            for (int x = 0; x < R_star.size(); ++x)
            {
                ...
                for (int i = 0; i < R_star[x].size(); ++i)
                {
                    ...
                    {
                        if (METHOD_VERBOSE)
                            std::cout << "\t\tR*: " << R_star[x][i] << " R_: " << V[i]->r << std::endl;
