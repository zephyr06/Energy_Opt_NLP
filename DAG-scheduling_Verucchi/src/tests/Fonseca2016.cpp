...
namespace dagSched
{
    ...
    float computeWCRT(const float base, const std::vector<int> &self_ss, const std::vector<int> &hp_ss, const int core_id, const Taskset &taskset, const int task_idx)
    {
        ...
        if (METHOD_VERBOSE)
            std::cout << "self_int: " << self_int << " high int: " << high_int << std::endl;
    ...
    void pathAnalysis(const std::vector<int> &path_ss, const std::vector<int> &self, const Taskset &taskset, const int task_idx, std::vector<std::vector<float>> &RTs, const bool joint, bool is_root)
    {
        //algorithm 1
        if (METHOD_VERBOSE)
            std::cout << "starting path analysis" << std::endl;
        if (METHOD_VERBOSE)
        ...
        if (METHOD_VERBOSE)
            printVector<int>(hp_ss, "hp tasks");
        if (METHOD_VERBOSE)
            printVector<int>(self_ss, "self ss");
        ...
        if (path_ss.size() == 1)
        {
            ...
            if (METHOD_VERBOSE)
                std::cout << "RT " << first->id << ", " << last->id << ": " << new_R_path_ss << std::endl;
        }
        else
        {
    ...
    float computeWCRTDAG(const Taskset &taskset, const int task_idx, const bool joint)
    {
        ...
        for (int i = 0; i < RTs.size(); ++i)
        {
            ...
            {
                if (METHOD_VERBOSE)
                    std::cout << "i: " << i << " j: " << j << " RT:" << RTs[i][j] << std::endl;
                if (RTs[i][j] > R_DAG)
                    R_DAG = RTs[i][j];
            }
        }
