...
namespace dagSched
{
    ...
    // J_nodes: true task index -> {all predecessors, all the levels up}
    ...
    std::vector<SPNode *> SPTree::computePSubtrees(const std::vector<std::vector<int>> &J_nodes, std::vector<STempNode> &S)
    {
        ...
        for (int i = 0; i < S.size(); ++i)
        {
            if (!S[i].visited)
            {
                ...
                // Sen: should visit for all the nodes? i.e., j=0,j!=i,j<n
                ...
                for (int j = 0; j < subtrees.size(); ++j)
                {
                    if (i != j && !used[i] && !used[j])
                    {
                        ...
                        for (int ii = 0; ii < Vids_i.size(); ++ii)
                        {
                            ...
                            {
                                if (Vids_i[ii].first == Vids_j[jj].first)
                                { // common id
                                    // merge
    ...
    void SPTree::convertNFJDAGtoSPTree(const DAGTask &dag, const int dag_id)
    {
        ...
        // for (size_t i = 0; i < J_nodes.size(); i++)
        // {
        //     for (size_t j = 0; j < J_nodes[i].size(); j++)
        //     {
        //         std::cout << J_nodes[i][j] << ", ";
        //     }
        //     std::cout << std::endl;
        // }
